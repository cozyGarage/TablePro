...
| TLS modes | Partial | Release-verified on PostgreSQL, including `VerifyFull` through SSH. Release-verified on MySQL, MongoDB, and Redis through the driver TLS fixture. Mapped but untested on SQL Server, and collapsed to on/off on ClickHouse. Saved connections carry a certificate authority. See [docs/connections.md](docs/connections.md) |
